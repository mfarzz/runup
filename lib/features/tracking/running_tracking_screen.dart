...
class _RunningTrackingScreenState extends State<RunningTrackingScreen> {
  ...
  void dispose() {
    ...
    if (existingSession != null && 
        existingSession.type == ActivityType.running &&
        (existingSession.status == ActivityStatus.active || existingSession.status == ActivityStatus.paused)) {
      print('RunningTracking: Found existing session - ${existingSession.id}');
  ...
  void _updateMapData() {
    ...
    _polylines = {
      Polyline(
        ...
        color: ActivityColors.running, // Using consistent running color
  ...
  void _showPermissionDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'This app needs location permission to track your running activity. Please enable location services and grant permission.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  void _startTracking() async {
    ...
    final success = await _locationService.startActivity(ActivityType.running);
    if (!success) {
      if (mounted) {
        _showPermissionDialog();
      }
  ...
  void _stopTracking() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Stop Running', style: TextStyle(color: Colors.white)),
        content: const Text(
          'Are you sure you want to stop this running session?',
          style: TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel', style: TextStyle(color: ActivityColors.running)),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(context);
      ...
      appBar: AppBar(
        ...
        elevation: 0,        leading: IconButton(
          onPressed: () {
            ...
            if (mounted) {
              context.go('/home');
              // Show floating widget after navigation with longer delay for context to be ready
        ...
        title: Container(
          ...
          child: Text(
            _currentSession?.activityTypeText ?? 'Running',
        ...
        actions: [
          ...
          IconButton(
            ...
            icon: Container(
              ...
              decoration: BoxDecoration(
                color: _autoFollow 
                    ? ActivityColors.running.withOpacity(0.3)
                    : Colors.black.withOpacity(0.3),
                borderRadius: BorderRadius.circular(12),
                border: _autoFollow 
                    ? Border.all(color: ActivityColors.running, width: 1)
                    : null,
              ),
              child: Icon(
                Icons.my_location,
                color: _autoFollow ? ActivityColors.running : Colors.white,
                size: 20,
              ),
            ),
          ),
    ...
    if (_currentSession == null) {
      return Row(
        ...
        children: [
          // Start Button
          GestureDetector(
            onTap: _isReady ? _toggleTracking : null,
            child: Container(
              width: 100,
              height: 100,
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: _isReady 
                      ? ActivityColors.getGradientColorsForActivity(ActivityType.running)
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: (isActive ? Colors.orange : ActivityColors.running).withOpacity(0.3),
                  blurRadius: 15,
                  offset: const Offset(0, 8),
                ),
                BoxShadow(

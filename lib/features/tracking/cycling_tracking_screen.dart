...
class CyclingTrackingScreen extends StatefulWidget {
  const CyclingTrackingScreen({super.key});

  @override
  State<CyclingTrackingScreen> createState() => _CyclingTrackingScreenState();
}

class _CyclingTrackingScreenState extends State<CyclingTrackingScreen> {
  ...
  Future<void> _initializeTracking() async {
    ...
    if (existingSession != null && 
        existingSession.type == ActivityType.cycling &&
        (existingSession.status == ActivityStatus.active || existingSession.status == ActivityStatus.paused)) {
      print('CyclingTracking: Found existing session - ${existingSession.id}');
  ...
  void _updateMapData() {
    ...
    _polylines = {
      Polyline(
        polylineId: const PolylineId('cycling_route'),
        points: _currentSession!.route,
        color: ActivityColors.cycling, // Using consistent cycling color
  ...
  void _showPermissionDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'This app needs location permission to track your cycling activity. Please enable location services and grant permission.',
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  void _startTracking() async {
    ...
    final success = await _locationService.startActivity(ActivityType.cycling);
    if (!success) {
      if (mounted) {
        _showPermissionDialog();
      }
  ...
  void _stopTracking() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Stop Cycling', style: TextStyle(color: Colors.white)),
        content: const Text(
          'Are you sure you want to stop this cycling session?',
          style: TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel', style: TextStyle(color: Color(0xFF32D74B))),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(context);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Container(
          ...
          child: Text(
            _currentSession?.activityTypeText ?? 'Cycling',
          ...
          IconButton(
            ...
            icon: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: _autoFollow 
                    ? Border.all(color: const Color(0xFF32D74B), width: 1)
                    : null,
              ),
              child: Icon(
                Icons.my_location,
                color: _autoFollow ? const Color(0xFF32D74B) : Colors.white,
                size: 20,
              ),
            ),
          ),
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: (_isReady && !_isOverlayActive)
                      ? [const Color(0xFF32D74B).withOpacity(0.8), const Color(0xFF32D74B)]
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: (isActive ? Colors.orange : const Color(0xFF32D74B)).withOpacity(0.3),
                    blurRadius: 15,
                    offset: const Offset(0, 8),
                  ),
                  BoxShadow(
